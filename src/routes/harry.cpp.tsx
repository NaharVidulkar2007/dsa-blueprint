...
function HarryCppLayout() {
  ...
  useEffect(() => {
    ...
    const target = (last && HARRY_CPP_LECTURES.find((l) => l.id === last))
      ?? HARRY_CPP_LECTURES[0];
      ...
      <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-white/8 bg-zinc-950/95 px-4 backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-orange-500/20 ring-1 ring-orange-500/30">
      ...
      {isEmpty ? (
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="max-w-sm text-center">
            <div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-orange-500/15 ring-1 ring-orange-500/30">
              <Sparkles className="h-5 w-5 text-orange-400" />
            </div>
            <h2 className="text-base font-semibold text-zinc-200">
              C++ lectures coming soon
            </h2>
            <Link
              to="/harry"
              className="mt-5 inline-flex items-center gap-1.5 rounded-lg bg-orange-500/15 px-4 py-2 text-sm font-medium text-orange-300 ring-1 ring-orange-500/30 hover:bg-orange-500/25 transition"
            >
...
function HarrySidebar({
  ...
  const activeTopic = useMemo(
    () => HARRY_CPP_LECTURES.find((l) => l.id === activeLectureId)?.topicId,
    [activeLectureId]
  );
  const [openTopics, setOpenTopics] = useState<Set<string>>(
    () => new Set(activeTopic ? [activeTopic] : [HARRY_CPP_TOPICS[0].id])
  );
    ...
    <div className="flex flex-col min-h-0 flex-1">
      <div className="px-3 py-2 border-b border-white/8">
        <input
          ...
          className="w-full rounded-lg bg-white/5 px-3 py-1.5 text-xs text-zinc-300 placeholder-zinc-600 outline-none ring-1 ring-white/10 focus:ring-orange-500/50"
        />
      </div>
      <div className="flex-1 overflow-y-auto py-2">
      {HARRY_CPP_TOPICS.map((topic) => {
        const allLectures = HARRY_CPP_BY_TOPIC[topic.id] ?? [];
        const lectures = q
                ...
                {lectures.map((lecture) => {
                  ...
                  return (
                    <li key={lecture.id}>
                      <button
                        onClick={() => {
                          navigate({ to: "/harry/cpp/$lectureId", params: { lectureId: lecture.id } });
                          onSelect?.();
                        }}
                      ...
                      >
                        {isDone ? (
                          ...
                          <Circle className={`mt-0.5 h-3.5 w-3.5 shrink-0 ${isActive ? "text-orange-400" : "text-zinc-600"}`} />
                        )}
                        <div className="min-w-0">
                          <p className={`text-[12px] leading-snug ${isActive ? "text-orange-300 font-medium" : isDone ? "text-zinc-500" : "text-zinc-300"}`}>
                            {lecture.title}
                          </p>
