...
function HarryJavaLayout() {
  ...
  useEffect(() => {
    ...
    const target = (last && HARRY_JAVA_LECTURES.find((l) => l.id === last))
      ?? HARRY_JAVA_LECTURES[0];
      ...
      {isEmpty ? (
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="max-w-sm text-center">
            <div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-amber-500/15 ring-1 ring-amber-500/30">
            ...
            <Link
              to="/harry"
              className="mt-5 inline-flex items-center gap-1.5 rounded-lg bg-amber-500/15 px-4 py-2 text-sm font-medium text-amber-300 ring-1 ring-amber-500/30 hover:bg-amber-500/25 transition"
            >
...
function HarrySidebar({
  ...
  const [openTopics, setOpenTopics] = useState<Set<string>>(
    () => new Set(activeTopic ? [activeTopic] : [HARRY_JAVA_TOPICS[0].id])
  );
                ...
                {lectures.map((lecture) => {
                  ...
                  return (
                    <li key={lecture.id}>
                      <button
                        onClick={() => {
                          navigate({ to: "/harry/java/$lectureId", params: { lectureId: lecture.id } });
                          onSelect?.();
                        }}
                      ...
                      >
                        {isDone ? (
                          ...
                          <Circle className={`mt-0.5 h-3.5 w-3.5 shrink-0 ${isActive ? "text-amber-400" : "text-zinc-600"}`} />
                        )}
                        <div className="min-w-0">
                          <p className={`text-[12px] leading-snug ${isActive ? "text-amber-300 font-medium" : isDone ? "text-zinc-500" : "text-zinc-300"}`}>
                            {lecture.title}
                          </p>
